...
function CSPS.applySkills(skipDiag, callOnSuccess, callOnFail, ignoreStyles)
    ...
    if profileXP then
        ...
        local prioXP = (profileKey == "0") or (profileKey == "1")

        local morphs, _, _, _, _, subclasses = CSPS.skTableExtract(profileXP.werte.prog)
        ...
        local skillTableBackup = {}

        d("[CARO TWEAK] profileKey: " .. profileKey)

        for _, skillData in pairs(morphs) do
		    local skillType, skillLineIndex, skillIndex, morph = unpack(skillData)
            -- if skillType ~= 1 or subClassesInBuild[skillLineIndex] or CSPS.savedVariables.settings.showAllClassSkills then
            -- Fill in the table entry
            local theSkill = skillTable[skillType][skillLineIndex][skillIndex]
            if theSkill then
                table.insert(skillTableBackup, {skillType, skillLineIndex, skillIndex, theSkill.rank, theSkill.purchased, theSkill.morph})
                theSkill.rank = 1
                theSkill.purchased = true
                morph = morph or 0
                theSkill.morph = theSkill.morph or 0
                if theSkill.morph == 0 or (prioXP and morph > 0) then
                    theSkill.morph = morph
                end
            end
            -- end
	    end

        local after = function (success)
            d("[CARO TWEAK] reset skill table")
            -- skillTable = skillTableBackup
            for _, skillData in pairs(skillTableBackup) do
                local skillType, skillLineIndex, skillIndex, rank, purchased, morph = unpack(skillData)
                local theSkill = skillTable[skillType][skillLineIndex][skillIndex]
                if theSkill then
                    theSkill.rank = rank
                    theSkill.purchased = purchased
                    theSkill.morph = morph
                end
            end
            ...
            zo_callLater(function()
                CSPS.refreshSkillSumsAndErrors()
                CSPS.refreshTree()
                if success and AG and profileKey then
                    ...
                    for i, profileAG in ipairs(AG.setdata.profiles) do
                        if profileAG.sortKey == profileKey then
                            d("[CARO TWEAK] load AG profile: " .. i)
                            AG.LoadProfile(i)
                            if (profileKey == "3") then -- solo build
                                AG.LoadSet(4)
        ...
        local callOnSuccessOrg, callOnFailOrg = callOnSuccess, callOnFail
        callOnSuccess = function () after(true) callOnSuccessOrg() end
        callOnFail = function () after(false) callOnFailOrg() end
    end
		...
		ZO_Dialogs_ShowDialog(CSPS.name.."_OkCancelDiag", 
				{
                    ...
                    cancelFunc = callOnFail -- TWEAK HERE
				},
				{
