...
function CSPS.groupApply(myGroupId)
	...
	for i=1, 12 do		
		...
		if mySk ~= nil and mySk ~= 0 then 
			currentHotbar[CSPS.cp2Disci[mySk]][mySk] = true 
		end
	end
	for i=1, 3 do
		if myGroup[i] ~= nil then
			...
			for j, w in pairs(CSPS.cp2HbTable[i]) do
				if not WouldChampionSkillNodeBeUnlocked(w, GetNumPointsSpentOnChampionSkill(w)) then
					local foundAlternative = false
					for k, x in pairs(currentHotbar[i]) do
						if not newHotbar[k] then
							CSPS.cp2HbTable[i][j] = k 
							newHotbar[k] = true 
							foundAlternative = true
							break 
						end
					end
					if not foundAlternative then CSPS.cp2HbTable[i][j] = nil end
				end
			end
			for j, w in pairs(CSPS.cp2HbTable[i]) do
				if not currentHotbar[i][w] then 
					needToApply = true 
				end
			end
			CSPS.cp2HbIcons(i)
	...
	if not groupContainsProfiles then d(string.format("[CSPS] %s", GS(CSPS_CPNoChanges))) return end
	CSPS.cp2UpdateHbMarks()
	CSPS.unsavedChanges = true
	CSPS.showElement("apply", true)
	CSPS.showElement("save", true)
	 CSPS.refreshTree()
	if needToApply then CSPS.cp2ApplyConfirm(false, hotbarsOnly) else d(string.format("[CSPS] %s", GS(CSPS_CPNoChanges))) end
	for i=1, 3 do
...
function CSPS.bindingsDR()
	...
	CSPSWindowManageBarsBtnDRBG:SetCenterColor(r,g,b, 0.4)
...
function CSPS.bindingsAG()
	...
	CSPSWindowManageBarsAddBind:SetHidden(true)
	local r,g,b = colTbl.orange:UnpackRGB()
	CSPSWindowManageBarsBtnAGBG:SetCenterColor(r,g,b, 0.4)
...
function CSPSbindingsList:BuildMasterList( )
	...
	for i,v in pairs(myBindingList) do
		if v[2] == 1 then
			...
			local setName = currentPage and currentPage.customSetName[v[4]]
...
local function HookDR(mySetId)
	...
	d(string.format("[CSPS] %s %s", GS(CSPS_CPLoadGroup), myKBGroup))
